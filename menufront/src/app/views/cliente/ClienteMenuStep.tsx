import { ChefHat, LogOut, ShoppingCart, Plus, Minus, X, Trash2 } from "lucide-react";
        ...
        <div className="min-h-screen bg-background pb-24 lg:pb-0 font-['Plus_Jakarta_Sans',sans-serif]">
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="mb-5">
                            ...
                            <p className="text-muted-foreground text-sm mt-0.5">
                                Comanda <span className="font-bold text-foreground">#{numeroComanda}</span> · Adicione itens ao carrinho
                            </p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {produtosFiltrados.map((produto) => {
                                ...
                                return (
                                    <div key={produto.id} className="bg-card rounded-2xl border border-border p-5 flex flex-col gap-4 hover:border-primary/30 hover:shadow-sm transition-all">
                                        <div>
                      <span className={`inline-block text-[10px] font-extrabold px-2 py-0.5 rounded-full mb-2 tracking-wide ${estilo.pill}`}>
                        {produto.categoria.toUpperCase()}
                                        ...
                                        <div className="flex items-center justify-between mt-auto">
                                            <span className="text-xl font-extrabold text-primary">{formatarPreco(produto.preco)}</span>
                                            {itemNoCarrinho ? (
                                                <div className="flex items-center gap-2.5">
                                                    ...
                                                    <span className="w-5 text-center font-extrabold text-foreground text-sm">{itemNoCarrinho.quantidade}</span>
                                                    <button
                        ...
                        <div className="bg-card rounded-2xl border border-border overflow-hidden sticky top-20">
                            <div className="px-5 py-4 border-b border-border flex items-center justify-between">
                                ...
                                {quantidadeCarrinho > 0 && (
                                    <span className="bg-primary text-white text-xs font-extrabold px-2 py-0.5 rounded-full">{quantidadeCarrinho}</span>
                                )}
                            </div>
                            {carrinho.length === 0 ? (
                                ...
                                <>
                                    <div className="p-4 space-y-2 max-h-72 overflow-y-auto">
                                        {carrinho.map((item) => (
                                            <div key={item.produto.id} className="flex items-center gap-2.5 py-1.5 border-b border-border/40 last:border-0">
                                                <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex items-center gap-1">
                                                    <button onClick={() => onRemoverDoCarrinho(item.produto.id)} className="w-6 h-6 rounded-full bg-muted flex items-center justify-center hover:bg-muted/70 transition-colors">
                                                        <Minus className="w-2.5 h-2.5" />
                                                    </button>
                                                    <span className="w-5 text-center text-xs font-extrabold">{item.quantidade}</span>
                                                    <button onClick={() => onAdicionarAoCarrinho(item.produto)} className="w-6 h-6 rounded-full bg-muted flex items-center justify-center hover:bg-muted/70 transition-colors">
                                                        <Plus className="w-2.5 h-2.5" />
                                                    </button>
                                                </div>
                                                <button onClick={() => onExcluirDoCarrinho(item.produto.id)} className="text-muted-foreground hover:text-red-500 transition-colors ml-0.5">
                                    ...
                                    <div className="px-4 pb-4 pt-2 border-t border-border bg-muted/20">
                                        <div className="flex items-center justify-between mb-3">
                                            ...
                                            <span className="font-extrabold text-base text-foreground">{formatarPreco(totalCarrinho)}</span>
                                        </div>
                                        <button
            ...
            {carrinho.length > 0 && (
                <div className="lg:hidden fixed bottom-0 inset-x-0 z-40">
                    ...
                    >
                        <div className="flex items-center gap-3">
                            <span className="w-7 h-7 rounded-full bg-white/20 flex items-center justify-center text-sm font-extrabold">{quantidadeCarrinho}</span>
                ...
                <div className="lg:hidden fixed inset-0 z-50 flex flex-col justify-end">
                    ...
                    <div className="relative bg-card rounded-t-3xl flex flex-col max-h-[85vh] overflow-hidden">
                        <div className="px-5 py-4 border-b border-border flex items-center justify-between">
                            ...
                            <button onClick={onFecharCarrinhoMobile} className="text-muted-foreground hover:text-foreground">
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        <div className="flex-1 overflow-y-auto p-4 space-y-2">
                            {carrinho.map((item) => (
                                <div key={item.produto.id} className="flex items-center gap-3 py-2 border-b border-border/40 last:border-0">
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm font-semibold text-foreground truncate">{item.produto.nome}</p>
                                        <p className="text-xs text-muted-foreground">{formatarPreco(item.produto.preco)} un.</p>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <button onClick={() => onRemoverDoCarrinho(item.produto.id)} className="w-7 h-7 rounded-full bg-muted flex items-center justify-center hover:bg-muted/70 transition-colors">
                                            <Minus className="w-3 h-3" />
                                        </button>
                                        <span className="w-5 text-center text-sm font-extrabold">{item.quantidade}</span>
                                        <button onClick={() => onAdicionarAoCarrinho(item.produto)} className="w-7 h-7 rounded-full bg-muted flex items-center justify-center hover:bg-muted/70 transition-colors">
                                            <Plus className="w-3 h-3" />
                                        </button>
                                    </div>
                                    <button onClick={() => onExcluirDoCarrinho(item.produto.id)} className="text-muted-foreground hover:text-red-500 transition-colors">
                        ...
                        <div className="p-4 border-t border-border bg-muted/20">
                            <div className="flex items-center justify-between mb-3">
                                ...
                                <span className="font-extrabold text-xl text-foreground">{formatarPreco(totalCarrinho)}</span>
                            </div>
                            <button
                                onClick={onFinalizarPedido}
